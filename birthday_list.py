...
def sort_key(birthday):
    return birthday['party_date']


def assemble_birthday_list(birthdays: list, reference_date: datetime, past_days: int, future_days: int) -> list:
    """Return sorted list of birthdays within a range from the reference date"""
    near_birthdays = []
    ...
    near_birthdays.sort(key=sort_key)
...
if __name__ == "__main__":
    ...
    bd4 = Birthday(name='martin', month=11, day=28)
    birthdays = [bd1, bd2, bd3, bd4]
    print(repr(birthdays))

    today = datetime(2022, 12, 4)

    bl = assemble_birthday_list(birthdays, today, 6, 2)
    for party in bl:
        print(party)
